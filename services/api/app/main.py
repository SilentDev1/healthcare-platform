...
async def request_logging(
    ...
    if request.method in {"GET", "POST"} and request.url.path.startswith(_RATE_LIMITED_PREFIXES):
        ...
        request_limit = (
            api_settings.ai_rate_limit_requests
            if request.url.path.startswith("/api/v1/ai/")
            else api_settings.rate_limit_requests
        )
        window_seconds = (
...
# Imported last so the AI route can safely call the already-defined deterministic
# comparison function without introducing a circular module dependency.
