"""Wave 7 NH chiropractic ingestion: verified data, capability, idempotency, no overreach."""
...
from scripts.ingest_nh_chiropractic_wave7 import VERIFIED_CHIRO, ingest
...
def test_ingests_verified_chiropractic_with_capability_and_provenance() -> None:
    session = _session()
    result = ingest(session)
    assert result["facilities"] == len(VERIFIED_CHIRO)
    ...
    for org in session.scalars(select(Organization)):
        assert org.organization_type == "chiropractic"
