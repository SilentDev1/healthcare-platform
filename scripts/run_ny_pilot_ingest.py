"""Bounded NY hospital-price ingestion (download → import → summaries) — NO auto-discovery.

Runs only the download + import steps for ``--state NY``. Because ``download_all_sources`` and
``import_all_sources`` act on facilities that already have a registered ``FacilityPriceSource``,
seeding sources for only the wave CCNs (via scripts.seed_ny_price_sources --only-ccns) keeps this
strictly bounded to that set. The auto-discovery web crawl in ``pipeline_full`` is deliberately
skipped so a wave cannot silently expand to unseeded hospitals.

``import_all_sources`` seeds the shared national ProcedureCodeMapping catalog and rebuilds price
summaries; ``rebuild_price_summaries`` preserves NH + MA hospital summaries and NH provider-published
summaries (the patched behaviour), so earlier markets are never damaged. Verify invariants with
scripts.phase_4_7_safety after each wave.

Large MRFs run SOLO (one CCN per job, generous timeout). Multi-campus single-CCN files import once
per CCN — see the ledger's shared_files (NYP 133957095, Montefiore 131740114, Garnet 146049030):
those CCNs share a source file, so import the file once and let the reviewed mappings attribute it;
never register the same file under multiple CCNs as if they were independent prices.

Run: python -m scripts.run_ny_pilot_ingest [--only-ccns ...] [--no-rebuild] [--rebuild-only]
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Bounded NY hospital-price ingestion (download+import+rebuild)")
    ...
    with session_factory() as session:
        if args.rebuild_only:
            ...
            print("NY_PILOT_INGEST=" + json.dumps(results, indent=2, default=str))
            return
